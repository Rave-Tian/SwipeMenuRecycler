...
//        mActionCallback.onMoved(fromPos, toPos);
    ...
    public void clearView(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
        ...
        if(viewHolder.itemView instanceof SwipeMenuView){
            ((SwipeMenuView)viewHolder.itemView).resetView();
        }
    }
    ...
    public void onChildDraw(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
            if (viewHolder.itemView != null && viewHolder.itemView instanceof SwipeMenuView && Math.abs(((SwipeMenuView) viewHolder.itemView).getCurrentDx() - dX) > 0) {
                ((SwipeMenuView) viewHolder.itemView).operateChildDraw(dX);
            }
        } else {
    ...
    public void onRecoveryPreOpen(@NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder, float dX) {
        ...
        if (viewHolder.itemView != null && viewHolder.itemView instanceof SwipeMenuView) {
            ((SwipeMenuView) viewHolder.itemView).operateChildDraw(dX);
        }
    }
