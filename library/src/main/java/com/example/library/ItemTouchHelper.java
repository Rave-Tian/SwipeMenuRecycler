...
public class ItemTouchHelper extends RecyclerView.ItemDecoration
    ...
    public static final int MENU_CLICKED_ITEM_TAG = Integer.MAX_VALUE;
    /**
    ...
    /**
     * Horizontal left direction. Resolved to item over scroll. you should set {@link #LEFT} as pre-condition,used for swipe & drag control.
     * 0100 0000
     */
    public static final int OVER_SCROLL_LEFT = START << 2;

    /**
     * Horizontal right direction. Resolved to item over scroll. you should set {@link #RIGHT} as pre-condition,used for swipe & drag control.
     * 1000 0000
     */
    public static final int OVER_SCROLL_RIGHT = END << 2;
    ...
    private static final int EXTEND_STATE_IDLE = 0;

    private static final int EXTEND_STATE_LEFT = 1;

    private static final int EXTEND_STATE_RIGHT = 2;

    private int currentExtendState = EXTEND_STATE_IDLE;

    float mLastOperateFinishX;

    float mLastOperateFinishY;
    ...
    float mInitialTouchXInScreen;

    float mInitialTouchYInScreen;
    ...
    /**
     * when a view is swiped, need to close the preview open menu item with an animation.
     */
    private ItemResetAnimation preItemResetAnimation;
    ...
    private final OnItemTouchListener mOnItemTouchListener = new OnItemTouchListener() {
        ...
        public boolean onInterceptTouchEvent(@NonNull RecyclerView recyclerView,
            ...
            if (action == MotionEvent.ACTION_DOWN) {
                ...
                mInitialTouchXInScreen = mInitialTouchX;
                mInitialTouchYInScreen = mInitialTouchY;
                obtainVelocityTracker();
                if (mSelected == null) {
                    // 如果触摸的Item之前触发过那么将会缓存到 mRecoverAnimations
                    final RecoverAnimation animation = findAnimation(event);
                    currentExtendState = EXTEND_STATE_IDLE;
                    mLastOperateFinishX = 0;
                    mLastOperateFinishX = 0;
                    if (animation != null) {
                        // 通过动画状态来判断当前的状态（如果animation.mX < 0,说明右侧菜单栏是打开状态 ）
                        if (animation.mX == 0) {
                            currentExtendState = EXTEND_STATE_IDLE;
                        } else if (animation.mX < 0) {
                            currentExtendState = EXTEND_STATE_RIGHT;
                        } else {
                            currentExtendState = EXTEND_STATE_LEFT;
                        }
                        mLastOperateFinishX = animation.mX;
                        mLastOperateFinishY = animation.mY;
                        mInitialTouchX -= animation.mX;
                        mInitialTouchY -= animation.mY;
                        endRecoverAnimation(animation.mViewHolder, true);
                        if (currentExtendState == EXTEND_STATE_IDLE) {
                            // 如果当前是菜单展开的状态，就不进行clear操作（如果这时clearView置为初始状态，dx要先变成0，会出现闪烁）
...
//            return (isClick)? false:mSelected != null;// mSelected == null,就返回false抛给RecyclerView处理，否则返回true就拦截手势
        }
    ...
    void select(@Nullable ViewHolder selected, int actionState) {
        if (selected == mSelected && actionState == mActionState) {// itemClick的情况，手势UP:selected == null; item菜单展开的时候，手势UP:selected == 当前展开的Item.
            return;
        }
        ...
        if (mSelected != null) {// mSelected 是上一个进行手势的View
...
//                        targetTranslateX = 0;
                ...
                } else if (swipeDir > 0 && swipeDir != OVER_SCROLL_RIGHT) {
                ...
                final RecoverAnimation rv = new RecoverAnimation(prevSelected, animationType,
                    ...
                    public void onAnimationEnd(Animator animation) {
                        ...
                        if (swipeDir <= 0 || swipeDir == OVER_SCROLL_RIGHT) {
        ...
        if (selected != null) {
            // 对面mSelected 进行赋值（用已经锁定的Item）
            mSelectedFlags =
                    (mCallback.getAbsoluteMovementFlags(mRecyclerView, selected) & actionStateMask)
                            >> (mActionState * DIRECTION_FLAG_COUNT);
            mSelectedStartX = selected.itemView.getLeft();//是否可以通过这个mSelectedStartX的坐标来判断当前的展开状态
    ...
    void postDispatchMenuItemClick(final ViewHolder viewHolder, final MotionEvent event) {

        final View view = findClickedMenuItem(viewHolder.itemView, event.getX(), event.getY());
        mRecyclerView.post(new Runnable() {
            @Override
            public void run() {
                if (mRecyclerView != null && mRecyclerView.isAttachedToWindow()
    ...
    public void onChildViewDetachedFromWindow(@NonNull View view) {
        ...
        if (mSelected != null && holder == mSelected) {
            ...
            if (this.preItemResetAnimation != null && this.preItemResetAnimation.mPreOpenedViewHolder == holder) {
                this.preItemResetAnimation.cancel();
            }
            // 当Item被回收重用的时候
    ...
    void checkSelectForSwipe(int action, MotionEvent motionEvent, int pointerIndex) {
        ...
        // findSwipedView根据手势坐标去RecyclerView里面找到对应的ViewHolder
        ...
        if (absDx > absDy) {// 判断当前设置的可支持滑动方向与我进行的手势是否匹配，如果不匹配直接return
    ...
    @SuppressWarnings("WeakerAccess")
    private boolean closePreOpenedItem() {
        if (mPreOpened == null || mPreOpened.itemView == null)
            return false;
...
//        final ViewHolder mFinalPreOpened = mPreOpened;
        preItemResetAnimation = new ItemResetAnimation(animation.mX, mPreOpened);
        preItemResetAnimation.start();
        return true;
    }
    ...
    RecoverAnimation findAnimation(View target) {
        if (mRecoverAnimations.isEmpty()) {
            return null;
        }
        for (int i = mRecoverAnimations.size() - 1; i >= 0; i--) {
...
//
        ...
        public boolean onSingleTapUp(MotionEvent e) {
            Log.e("tianrenzheng", "onSingleTapUp");
//            closePreOpenedItem();
    ...
    private class ItemResetAnimation implements Animator.AnimatorListener {
        final float mStartDx;
        final ViewHolder mPreOpenedViewHolder;
        private final ValueAnimator mValueAnimator;
        public boolean mEnded = false;

        private ItemResetAnimation(float mStartDx, ViewHolder mViewHolder) {
            this.mStartDx = mStartDx;
            this.mPreOpenedViewHolder = mViewHolder;
            mValueAnimator = ValueAnimator.ofFloat(mStartDx, 0f);
            mValueAnimator.addListener(this);
            mValueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                @Override
                public void onAnimationUpdate(ValueAnimator animation) {
                    mCallback.onRecoveryPreOpen(mRecyclerView, mPreOpenedViewHolder, (Float) animation.getAnimatedValue());
                }
            });
        ...
        public void onAnimationStart(Animator animation) {
            mEnded = false;
            if (mPreOpenedViewHolder != null) mPendingCleanup.remove(mPreOpenedViewHolder.itemView);
            endRecoverAnimation(mPreOpenedViewHolder, true);
        }

        @Override
        public void onAnimationEnd(Animator animation) {
//            if (!mEnded) {
//                mPreOpenedViewHolder.setIsRecyclable(true);
//            }
            mEnded = true;
//            mPreOpened = null;
